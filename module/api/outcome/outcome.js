...
export const outcome = ({ type = "outcome", title = "", description = "", ...props }) => () => ({
  id: randomID(),
  type,
  title,
  description, ...props
...
export const withTest = ({ dr = 12, fumbleOn = 1, critOn = 20 } = {}) => (outcome) =>
  asyncPipe(
    withProps({
      dr,
      fumbleOn,
...
export const withTarget = ({ actor, targetToken } = {}) => async (outcome) => ({
  ...outcome,
  initiatorToken: getInitiatorToken(actor)?.id,
  targetToken: targetToken?.id,
});

export const withButton = ({ title, type }) => async (outcome) => ({
  ...outcome,
  button: {
    title,
    data: {
...
export const withAutomations = (...types) => async (outcome) => ({
  ...outcome,
